...
const Tech = () => {
  return (
    ...
    >
      <h2 className="pb-2 text-2xl lg:text-[40px] font-bold w-full max-w-[1260px] text-shadow-sm">MY TOOLKIT</h2>
      <div className="flex flex-col max-w-[1260px] h-full w-full border-t border-white-faded mb-[100px]">
        <ul className="flex justify-center gap-10 flex-wrap py-10">
          {techData.map((tech) => (
            <li className="flex items-center gap-2 text-xl drop-shadow-sm drop-shadow-black/80" key={tech.id}>
              {tech.icon}
              <p>{tech.name}</p>
            </li>
          ))}
